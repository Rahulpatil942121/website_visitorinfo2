...
class Admincontroller extends Controller
{
    ...
    public function Add_Product()
    {
        ...
        $data['Company'] = Companyname::where(['status'=>1])->get();
        return view('/frontend/Webviews/Manageviews',$data); 
    }
    ...
    public function Update_Product(Request $req)
    {
        ...
        if(DB::table('productnames')->where(['product_name'=>$req->product_name,'portal_id'=>$req->ddlportal])->where('id','!=',$req->product_id)->doesntExist())
        {
            $result = DB::table('productnames')->where(['id'=>$req->product_id])->update(['product_name'=>$req->product_name,'portal_id'=>$req->ddlportal,
                ...
                'comp_name'=>$req->ddlcompany,'status'=>$req->status]);
            if($result)
            {
...
// ------------------------------------------
 ...
 public function Visitor_List()
 {
    ...
    if(Auth::User()->role == 1)
    {
        $product= Productname::where('comp_name',Auth::User()->comp_name)->select('id')->get(); 
        $data['Visitor'] = Visitorinfo::orderBy('id','DESC')->whereIn('product_id', $product)->get();         
    }
 ...
 public function Users_List()
 {
    ...
    $data['Users'] = User::where('role',1)->orderBy('id','DESC')->get();
    return view('/frontend/Webviews/Manageviews',$data);
 }
 ...
 public function Add_User()
 {
    ...
    $data['Usertype'] = Usertype::where(['id'=>1,'status'=>1])->get();
    $data['Company'] = Companyname::where(['status'=>1])->get();
    return view('/frontend/Webviews/Manageviews',$data);
 }
// -----------------------------
 public function Submit_Client(Request $req)
 {
    //dd($req);
    $validated = $req->validate([
            'ddltype' => 'required',
